...
public class Game {
	...
	public static void updateAliens(List<Alien> aliens, List<Bullet> bullets) {
		...
		for (Alien alien : aliens) {
			if (alien.getRx() < -0.1 || alien.getRy() < -0.1 || alien.getRy() > 1.1) {
				aliens.remove(alien);
				break;
			}
		}
	...
	public static void updateBullets(List<Bullet> bullets) {
		...
		for (Bullet bullet : bullets) {
			if (bullet.getRx() < -0.1 || bullet.getRx() > 1.1 || bullet.getRy() < -0.1 || bullet.getRy() > 1.1) {
	...
	public static void updateScore(int score) {
		...
		StdDraw.text(0.5, 0.05, "Score: " + score);
	}
	...
	public static void updateLives(int playerALives, int playerBLives) {
		StdDraw.setPenColor(StdDraw.BLACK);
		StdDraw.textRight(0.95, 0.05, "Player 1");
		StdDraw.textRight(0.95, 0.95, "Player 2");
		if (playerALives == 0) {
			StdDraw.textRight(0.95, 0.1, "Game Over");
		} else {
			StdDraw.textRight(0.95, 0.1, "Lives: " + playerALives);
		}
		if (playerBLives == 0) {
			StdDraw.textRight(0.95, 0.9, "Game Over");
		} else {
			StdDraw.textRight(0.95, 0.9, "Lives: " + playerBLives);
		}	
	}
