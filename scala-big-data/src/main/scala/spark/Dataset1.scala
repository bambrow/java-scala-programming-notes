...
object Dataset1 extends App {
  ...
  df1 foreach (println(_)) // will not send to driver, execute on workers only

  println

  df1.as[PersonWithAge] repartition 5 foreachPartition {
    x: Iterator[PersonWithAge] => println(x.toList) // will not send to driver, execute on workers only
  }
  /*
